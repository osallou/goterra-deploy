...
//nsCreatedMessage sends a message to rabbitmq exchange
func nsCreatedMessage(uid string, ns string) error {
	if os.Getenv("GOT_MOCK_AMQP") == "1" {
		return nil
	}
	config := terraConfig.LoadConfig()
	...
	msg := &terraModel.UserAction{Action: "ns_create", UID: uid, Data: ns}
...
//nsUpdatedMessage sends a message to rabbitmq exchange
func nsUpdatedMessage(uid string, ns string) error {
	if os.Getenv("GOT_MOCK_AMQP") == "1" {
		return nil
	}
	config := terraConfig.LoadConfig()
	...
	msg := &terraModel.UserAction{Action: "ns_update", UID: uid, Data: ns}
	body, _ := json.Marshal(msg)
	err = ch.Publish(
		"gotevent", // exchange
		"",         // routing key
...
//nsDeletedMessage sends a message to rabbitmq exchange
func nsDeletedMessage(uid string, ns string) error {
	if os.Getenv("GOT_MOCK_AMQP") == "1" {
		return nil
	}
	config := terraConfig.LoadConfig()
	...
	msg := &terraModel.UserAction{Action: "ns_delete", UID: uid, Data: ns}
	body, _ := json.Marshal(msg)
	err = ch.Publish(
		"gotevent", // exchange
		"",         // routing key
